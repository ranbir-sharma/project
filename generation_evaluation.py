...
def my_sample(model, gen_data_dir, sample_batch_size = 25, obs = (3,32,32), sample_op = sample_op):
    for label, label_value in my_bidict.items():
        ...
        condition = torch.full((sample_batch_size,), label_value, dtype=torch.long, device=device)
        sample_t = sample(model, sample_batch_size, obs, sample_op, condition=condition)
...
if __name__ == "__main__":
    ...
    model = PixelCNN(nr_resnet=1, nr_filters=40, input_channels=3, nr_logistic_mix=5, num_classes=len(my_bidict), embedding_dim=embedding_dim)
